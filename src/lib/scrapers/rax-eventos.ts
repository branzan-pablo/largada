/**
 * Scraper – Rax Eventos (raxeventos.com.br)
 *
...
function extractEventURLsFromSitemap(xml: string): string[] {
  ...
  for (const m of matches) {
    const url = m[1].trim();
    // Only event pages, skip homepage/sitemap links
    if (url.includes("/events/")) {
      urls.push(url);
    }
  }
  // If no /evento/ pattern, take all URLs from sitemap (it's event-specific)
  if (urls.length === 0) {
    const allMatches = xml.matchAll(/<loc>\s*(https?:\/\/[^<]+)\s*<\/loc>/gi);
...
function parseStartDate(dateStr: string): {
  ...
  // ISO 8601: "2026-05-10T07:00:00-03:00" or "2026-05-10"
...
async function scrapeEventPage(url: string): Promise<ScrapedRace | null> {
  ...
  if (!res.ok) {
    console.warn(`[rax-eventos] HTTP ${res.status} for ${url}`);
    return null;
  }
  ...
  if (!jsonLd || !jsonLd.name) {
    console.warn(`[rax-eventos] No JSON-LD Event found at ${url}`);
    return null;
  }
  ...
  return {
    ...
    organizer: "Rax Eventos",
    description: jsonLd.description?.slice(0, 500) || null,
    link: url,
  };
}

async function scrapeRaxEventos(): Promise<ScrapedRace[]> {
  ...
  if (eventUrls.length === 0) {
    console.warn("[rax-eventos] No event URLs found in sitemap");
    return [];
  }

  console.log(`[rax-eventos] Found ${eventUrls.length} event URLs in sitemap`);
  ...
  for (let i = 0; i < eventUrls.length; i += CONCURRENCY) {
    ...
    for (const result of results) {
      if (result.status === "fulfilled" && result.value) {
        ...
        console.warn("[rax-eventos] Event page failed:", result.reason);
      }
    }
  }
