...
function extractCidadeEstado(titulo: string): { cidade: string | null; estado: string | null } {
    ...
    if (fallback) return { cidade: fallback[1].trim(), estado: fallback[2].trim() };
...
function extractDataEvento(texto: string): string | null {
    const m1 = texto.match(/(?:dia|n[oa]?\s+dia)\s+(\d{1,2})\s+de\s+(\w+)\s+de\s+(\d{4})/i);
    if (m1) {
        const mes = MESES[m1[2].toLowerCase()];
        if (mes) return `${m1[3]}-${mes}-${m1[1].padStart(2, "0")}`;
    }
    const m2 = texto.match(/realizado\s+(?:no\s+)?dia\s+(\d{1,2})\s+de\s+(\w+)\s+de\s+(\d{4})/i);
    if (m2) {
...
function extractHorarioLargada(texto: string): string | null {
    const m = texto.match(/largada\s+(?:est[aá]\s+)?prevista\s+para\s+[àa]s?\s+(\d{1,2})[h:](\d{2})?/i);
...
function extractValorInscricao(texto: string): string | null {
    const lotes: string[] = [];
    const loteMatches = [...texto.matchAll(/(\d)[ºo°]\s*lote[^:]*?:\s*(?:.*?)R\$\s*(\d+(?:[.,]\d{2})?)/gi)];
...
function extractDataLimiteInscricao(texto: string): string | null {
    const m1 = texto.match(/encerrad[ao]s?\s+(?:via\s+internet\s+)?(?:no\s+)?dia\s+(\d{1,2})\s+de\s+(\w+)\s+de\s+(\d{4})/i);
    if (m1) {
        const mes = MESES[m1[2].toLowerCase()];
        if (mes) return `${m1[3]}-${mes}-${m1[1].padStart(2, "0")}`;
    }
...
function extractLocalLargada(texto: string): string | null {
    const m = texto.match(/largada\s+(?:e\s+chegada\s+)?(?:n[oa]\s+|será\s+n[oa]\s+)(.+?)(?:\.|,|\n)/i);
    if (m) {
        let local = m[1].trim().replace(/\s*[-–]\s*SP\s*$/, "");
        return local || null;
    }
...
function extractDescricao(texto: string): string | null {
    ...
    const m2 = texto.match(/O EVENTO\s*\n([\s\S]+?)(?:\n[A-Z]{3,}|\n\d+\s*[–\-.]\s)/);
    if (m2) {
...
function extractPrizeInfo(texto: string): { type: "money" | "trophy" | "both" | "none"; details: string | null } {
...
function extractRouteDescription(texto: string): string | null {
    const m = texto.match(/PERCURSO\s*\n([\s\S]+?)(?:\n[A-Z]{3,}|\n\d+\s*[–\-.])/);
...
function extractImageFromHTML(html: string): string | null {
    ...
    const img = html.match(/<img[^>]+src="(https?:\/\/equilibrio\.esp\.br\/wp-content\/uploads\/[^"]+)"/i);
    if (img && !img[1].includes("favicon")) return img[1];
    return null;
}

export async function scrapeEquilibrio(): Promise<ScrapedRace[]> {
    const corridasRes = await fetch(`${BASE_URL}/wp-json/wp/v2/corridas-proximos?per_page=100`);
    ...
    let produtosPorSlug: Record<string, WPProduct> = {};
    try {
        const produtosRes = await fetch(`${BASE_URL}/wp-json/wp/v2/product?per_page=100&_embed`);
        if (produtosRes.ok) {
    ...
    for (const corridaAPI of corridasAPI) {
        ...
        try {
            ...
            if (res.ok) { htmlDetalhe = await res.text(); textoDetalhe = stripHTML(htmlDetalhe); }
        } catch {
            try {
                const altSlug = slug.replace(/-sp$/, "-s");
                if (altSlug !== slug) {
                    const res = await fetch(`${BASE_URL}/${altSlug}/`);
                    if (res.ok) { htmlDetalhe = await res.text(); textoDetalhe = stripHTML(htmlDetalhe); }
                }
            } catch { }
        }
        ...
        const produto = produtosPorSlug[slug] ?? Object.values(produtosPorSlug).find((p) => slug.includes(p.slug) || p.slug.includes(slug));
        if (produto?.uagb_featured_image_src?.full?.[0]) image_url = produto.uagb_featured_image_src.full[0];
        ...
        races.push({
            name: titulo, slug, date: data, startTime: startTime || "07:00",
            city: cidade, state: estado || "SP", address, distances: distancias,
            registrationPrice: valorInscricao, registrationLink: linkInscricao || pageURL,
            registrationDeadline: dataLimite, prizeType, prizeDetails: null,
            image_url, routeDescription, organizer: "Equilíbrio Esportes",
            description: descricao, link: pageURL,
        });
    }
    return races;
}
