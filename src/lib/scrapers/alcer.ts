/**
 * Scraper – ALCER Eventos (alcer.esp.br)
 *
 * Busca corridas via sitemap XML → páginas de evento → JSON-LD structured data.
 * Same Wix platform as Rax Eventos — identical JSON-LD format.
 */
...
function extractEventURLsFromSitemap(xml: string): string[] {
  ...
  for (const m of matches) {
    const url = m[1].trim();
    // Only event pages, skip /form duplicates
    if (url.includes("/events/") && !url.endsWith("/form")) {
      urls.push(url);
    }
  }
  // If no /events/ pattern, take all URLs from sitemap (it's event-specific)
  if (urls.length === 0) {
    const allMatches = xml.matchAll(/<loc>\s*(https?:\/\/[^<]+)\s*<\/loc>/gi);
    for (const m of allMatches) {
      const u = m[1].trim();
      if (!u.endsWith("/form")) urls.push(u);
    }
  }
  return urls;
}
