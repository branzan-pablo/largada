...
export async function GET(request: Request) {
  ...
  if (!adminProfile)
    return NextResponse.json(
      { error: "Nenhum perfil admin encontrado" },
      { status: 500 },
    );
  ...
  for (const race of scraped) {
    ...
    if (race.city) {
      ...
      if (cityRow) {
        ...
        console.warn(
          `[scrape-races] City not found in DB: "${race.city}" — race "${race.name}" will have (0,0) coords`,
        );
      }
    }
    ...
    if (insertError) {
      console.error(
        `[scrape-races] Insert failed for "${race.name}":`,
        insertError.message,
      );
  ...
  if (errors.length > 0)
    console.warn("[scrape-races] Error details:", errors.join(" | "));
  console.log(
    `[scrape-races] Done: scraped=${scraped.length} inserted=${inserted} skipped=${skipped} errors=${errors.length}`,
  );
  return NextResponse.json({
