...
function buildShareText({
  ...
  startTime,
  distances,
}: Omit<RaceShareButtonProps, "shareUrl" | "className">) {
...
export function RaceShareButton(props: RaceShareButtonProps) {
  const { shareUrl, className } = props;
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent side="bottom" className="rounded-t-2xl pb-[max(1.5rem,env(safe-area-inset-bottom))]">
          <SheetHeader>
            ...
            <SheetDescription>
              Escolha onde deseja divulgar esta corrida.
            </SheetDescription>
          </SheetHeader>
          ...
          <div className="grid grid-cols-4 gap-3 px-4 pb-2">
            ...
            <button
              ...
              onClick={() =>
                copyToClipboard(
                  fullText,
                  "Texto copiado! Cole no seu story ou bio do Instagram."
                )
              }
              className="flex flex-col items-center gap-1.5 text-xs font-medium text-[#0D1B2A] hover:opacity-80 cursor-pointer"
            >
              <span className="flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-tr from-[#f09433] via-[#dc2743] to-[#bc1888] text-white">
